...
export default function ScenarioTest() {
  ...
  useEffect(() => {
    const fetchScenarios = async () => {
      try {
        const data = await tests.scenarios();
        const blocks = (data as unknown as ScenarioBlock[]) || [];
        setScenarios(blocks);
        ...
        setResponses(blocks.map((s) => ({ blockNumber: s.block_number, most: "", least: "" })));
      } catch {
  ...
  const handleFinish = useCallback(async () => {
    ...
    try {
      ...
      await tests.submitResponses(
        payload.map((p) => ({ block_number: p.block_number, most_option: p.most_option, least_option: p.least_option })) as any
      );
      await tests.calculate();
