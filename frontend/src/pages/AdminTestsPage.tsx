...
export default function AdminTestsPage() {
  ...
  const handleReprocess = async (testId: string) => {
    setReprocessingId(testId);
    try {
      await testsApi.reprocess(testId);
  ...
  const handleDelete = async (testId: string) => {
    ...
    try {
      await testsApi.deleteResult(testId);
  ...
  const { data: profiles } = useQuery({
    ...
    queryFn: () =>
      profilesApi.list() as Promise<Array<{ user_id: string; name: string; email: string; company_id: string | null }>>,
  });
  ...
  const { data: tests, isLoading } = useQuery({
    ...
    queryFn: () =>
      testsApi.results() as Promise<Array<{
        id: string;
        user_id: string;
        completed_at: string;
        disc_natural: unknown;
        disc_adapted: unknown;
        big_five: unknown;
      }>>,
  });
                        ...
                        {isMasterAdmin && (
                          <>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => handleReprocess(test.id)}
