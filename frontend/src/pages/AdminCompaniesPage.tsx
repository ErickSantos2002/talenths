...
import { companies as companiesApi, departments as departmentsApi, profiles as profilesApi } from "@/lib/api";
...
export default function AdminCompaniesPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [companiesData, deptsData, profilesData] = await Promise.all([
        companiesApi.list(),
        departmentsApi.list(),
        profilesApi.list(),
      ]);

      setCompaniesList((companiesData as Company[]) ?? []);

      const dc: Record<string, number> = {};
      (deptsData as Array<{ company_id: string }>).forEach((d) => {
        dc[d.company_id] = (dc[d.company_id] || 0) + 1;
      });
      setDeptCounts(dc);

      const pc: Record<string, number> = {};
      (profilesData as Array<{ company_id?: string }>).forEach((p) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        title: editingCompany ? "Erro ao atualizar empresa" : "Erro ao criar empresa",
