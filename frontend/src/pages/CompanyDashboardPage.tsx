...
import { profiles, departments, tests, invitations } from "@/lib/api";
...
export default function CompanyDashboardPage() {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [profilesData, deptsData, invitesData, allResults] = await Promise.all([
        profiles.list(companyId),
        departments.list(companyId),
        invitations.list(companyId),
        tests.results(),
      ]);
      ...
      const collabCount = allProfiles.length;
  ...
  const toggleInvite = async (inviteId: string, currentActive: boolean) => {
    try {
      await invitations.update(inviteId, { is_active: !currentActive });
      toast({ title: currentActive ? "Convite desativado" : "Convite reativado" });
      fetchAll();
    } catch (err: any) {
      toast({ title: "Erro ao atualizar convite", description: err.message, variant: "destructive" });
    }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    setCreating(true);
    try {
      await invitations.update(editInvite.id, {
      ...
      toast({ title: "Erro ao editar convite", description: err.message, variant: "destructive" });
    } finally {
      setCreating(false);
    }
  };
