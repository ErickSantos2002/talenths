...
export default function TeamAnalysisPage() {
  ...
  const load = useCallback(async () => {
    if (!effectiveCompanyId) { setMembers([]); setDepartments([]); setLoading(false); return; }
    setLoading(true);

    const [profilesRes, deptsRes, rolesRes] = await Promise.all([
      supabase.from("profiles").select("user_id, name, department_id").eq("company_id", effectiveCompanyId),
      supabase.from("departments").select("id, name").eq("company_id", effectiveCompanyId),
      supabase.from("user_roles").select("user_id, role").in("role", ["leader", "company_admin"]),
    ]);

    const profiles = profilesRes.data;
    if (!profiles || profiles.length < 2) { setMembers([]); setDepartments([]); setLoading(false); return; }

    setDepartments(deptsRes.data ?? []);

    const rolesData = rolesRes.data ?? [];
    const leaderIds = new Set(rolesData.filter((r) => r.role === "leader").map((r) => r.user_id));
    const adminIds = new Set(rolesData.filter((r) => r.role === "company_admin").map((r) => r.user_id));
    ...
    const { data: results } = await supabase
      .from("test_results")
      .select("user_id, disc_natural, big_five, completed_at")
      .in("user_id", userIds)
      .order("completed_at", { ascending: false });

    if (!results) { setMembers([]); setLoading(false); return; }

    const latestByUser = new Map<string, typeof results[0]>();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('team-analysis-realtime')
      .on(
        'postgres_changes',
      ...
      <div className="animate-fade-in space-y-6">
        ...
        <Tabs defaultValue="geral">
          ...
          <TabsContent value="departamento" className="space-y-6">
            <DepartmentAnalysis members={members} departments={departments} onCompare={modal.openModal} />
          </TabsContent>
