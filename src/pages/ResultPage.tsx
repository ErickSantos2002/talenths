...
export default function ResultPage() {
  ...
  useEffect(() => {
    const fetchResult = async () => {
      ...
      if (paramTestId) {
        query = query.eq("id", paramTestId);
      } else if (shareToken) {
        query = query.eq("share_token", shareToken);
      } else if (paramUserId) {
        query = query.eq("user_id", paramUserId).order("completed_at", { ascending: false });
      } else if (user) {
        query = query.eq("user_id", user.id).order("completed_at", { ascending: false });
      } else {
        setIsLoading(false);
        return;
      }
      ...
      if (error) {
        ...
        const r = data as unknown as TestResult;
        setResult(r);
        ...
        if (profile) {
          ...
          setProfileInfo({ name: profile.name, companyName });
        }
      }
      setIsLoading(false);
    };
  ...
  useEffect(() => {
    ...
    const fetchResponses = async () => {
      const [{ data: resp }, { data: scen }] = await Promise.all([
        supabase.from("test_responses").select("*").eq("user_id", result.user_id).order("block_number"),
        supabase.from("scenario_blocks").select("*").order("block_number"),
      ]);
      setResponses(resp || []);
      setScenarios(scen || []);
    };
