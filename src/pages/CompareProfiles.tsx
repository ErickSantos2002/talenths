...
export default function CompareProfiles() {
  ...
  // Fetch collaborators with test status and roles
  useEffect(() => {
    (async () => {
      const { data: results } = await supabase.from("test_results").select("user_id");
      const testedIds = new Set((results || []).map((r) => r.user_id));

      let profileQuery = supabase.from("profiles").select("user_id, name");
      if (effectiveCompanyId) profileQuery = profileQuery.eq("company_id", effectiveCompanyId);
      const { data: profiles } = await profileQuery;

      if (profiles) {
        // Fetch roles for all users
        const { data: rolesData } = await supabase.from("user_roles").select("user_id, role");
        const rolesByUserId = new Map(rolesData?.map((r) => [r.user_id, r.role]) || []);
        ...
        setAllCollaborators(
          profiles.map((p) => ({
            ...
            role: rolesByUserId.get(p.user_id) || "user",
          }))
        );
      }
    })();
  ...
  const handleCompare = useCallback(async (overrideUser1?: string, overrideUser2?: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("compare-profiles", {
        body: { user1_id: u1, user2_id: u2, comparison_type: comparisonType },
      });
